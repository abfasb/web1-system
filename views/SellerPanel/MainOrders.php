...
$query = "
    ...
    GROUP BY o.order_id, u.username, c.category_name
";
...
<body class="text-gray-800 font-inter">
    <div class="fixed left-0 top-0 w-64 h-full bg-[#f8f4f3] p-4 z-50 sidebar-menu transition-transform">
        ...
        <li class="mb-1 group">
            ...
            <li class="mb-1 group">
                <a href="../SellerPanel" class="flex font-semibold items-center py-2 px-4 text-gray-900 hover:bg-gray-950 hover:text-gray-100 rounded-md group-[.active]:bg-gray-800 group-[.active]:text-white group-[.selected]:bg-gray-950 group-[.selected]:text-gray-100 sidebar-dropdown-toggle">
                ...
                <ul class="pl-7 mt-2 hidden group-[.selected]:block">
                    <li class="mb-4">
                        <a href="./MainProduct.php" class="text-gray-900 text-sm flex items-center hover:text-[#f84525] before:contents-[''] before:w-1 before:h-1 before:rounded-full before:bg-gray-300 before:mr-3">All</a>
                    </li> 
                    <li class="mb-4">
    ...
    <div class="mx-auto max-w-screen-2xl px-4 lg:px-12">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="flex-1 flex items-center space-x-2">
                    <h5>
                        ...
                        <span class="dark:text-white">25</span>
                    </h5>
        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="p-4">Shipping Method</th>
                <th scope="col" class="p-4">Rating</th>
                <th scope="col" class="p-4">Price</th>
                <th scope="col" class="p-4">Status</th>
                <th scope="col" class="p-4">Confirmation</th>
            </tr>
        </thead>
        <tbody>
        <?php foreach ($orders as $order): ?>
            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                <td class="p-4 w-4">
                    <div class="flex items-center">
                        <input id="checkbox-table-search-<?= $order['order_id'] ?>" type="checkbox" onclick="event.stopPropagation()" class="w-4 h-4 text-primary-600 bg-gray-100 rounded border-gray-300 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        <label for="checkbox-table-search-<?= $order['order_id'] ?>" class="sr-only">checkbox</label>
                    </div>
                </td>
                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= htmlspecialchars($order['buyer_name']) ?></td>
                <td class="px-4 py-3"><div><?= htmlspecialchars($order['product_names']) ?></div></td>
                <td class="px-4 py-3"><?= htmlspecialchars($order['category_name']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($order['address']) ?></td>
                <td class="px-4 py-3"><?= htmlspecialchars($order['shipping_method']) ?></td>
                <td class="px-4 py-3">
                    <div class="flex items-center">
                        <?php for ($i = 0; $i < 5; $i++): ?>
                            <svg aria-hidden="true" class="w-5 h-5 <?= $i < $order['avg_rating'] ? 'text-yellow-400' : 'text-gray-300' ?>" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                        <?php endfor; ?>
                        <span class="text-gray-500 dark:text-gray-400 ml-1"><?= htmlspecialchars($order['avg_rating']) ?></span>
                    </div>
                </td>
                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">$<?= htmlspecialchars($order['max_price']) ?></td>
                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= htmlspecialchars($order['status']) ?></td>
                <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    <?php $isCompleted = ($order['status'] == 'active'); ?>
                    <div class="flex items-center space-x-4">
                        <button type="button" <?= $isCompleted ? 'disabled' : '' ?> onclick="confirmOrder(<?= $order['order_id'] ?>)" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300">
                            <?php if ($isCompleted): ?>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-4 h-4 mr-2" viewBox="0 0 24 24" aria-hidden="true">
                                    <path d="M12 2a10 10 0 00-4.074 19.18c.31.056.426-.134.426-.3v-1.17c-1.732.377-2.1-.785-2.224-1.51-.047-.268-.243-.704-.414-.844-.142-.118-.344-.404-.01-.412.32-.008.548.294.624.414.366.624.952.434 1.187.33.037-.264.142-.434.258-.534-1.54-.174-3.16-.772-3.16-3.432 0-.758.27-1.38.71-1.865-.071-.173-.308-.874.067-1.82 0 0 .58-.186 1.9.71a6.48 6.48 0 013.45 0c1.32-.896 1.9-.71 1.9-.71.376.946.139 1.647.068 1.82.44.486.71 1.107.71 1.865 0 2.67-1.62 3.257-3.16 3.43.15.13.284.384.284.774v1.15c0 .168.116.357.426.3A10.002 10.002 0 0012 2z"></path>
                                </svg>
...
<script>
  ...
  function confirmOrder(orderId) {
    $.ajax({
        url: '../../controllers/confirm_order.php',
        method: 'POST',
        data: { orderId: orderId },
        success: function(response) {
            alert("Ordered Accepted Successfully!");
            location.reload();
        },
